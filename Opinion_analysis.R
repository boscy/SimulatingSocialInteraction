...
filename <- "HUMAT_from_scratch_test_3.csv"
# filename_7 <-"HUMAT_Mental_health_test.csv"

dat <- fread(file = filename, skip = 29)  # skip 23 or 7 depending on which behaviorspace csv file chosen
...
vals_list <- lapply(dat$V3 ,function(x){    # Note that V2 could be V11, depending on Behaviorspace output
  y <- gsub("\\[|\\]", "", x)
...
ggplot(data=new_df, 
       aes(x=days, y=mean_o)) + 
         ...
         ylab('Mean Opinion') + 
         xlab('Time (days)') +
        geom_ribbon(aes(ymin=mean_o-sd_o, ymax=mean_o+sd_o), linetype=2, fill='cyan4', alpha=0.2) +
  ggtitle("Average opinion of HUMATs over time") +
