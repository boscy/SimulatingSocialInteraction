...
filename <- "HUMAT_COVID_visitors_measure_FINAL importances_sweep_end_values_100_runs.csv"
...
important_dat <- dat[,c("experiential-importance-parameter", "values-importance-parameter",
                        "allowed_contacts_per_day", "mean [n_contacts_this_day] of HUMATS", 
                        "percentage_in_dilemma", "standard-deviation [opinion_measure] of HUMATS",
                        "mean [opinion_measure] of HUMATS")]
...
colnames(important_dat) <- c("experiential_importance_parameter", "values_importance_parameter",
                             "allowed_contacts_per_day", "contacts", 
                             "dilemmas", "sd",
                             "opinion_mean")
...
ggplot(data_for_visualization, aes(x = experiential_importance_parameter, y = values_importance_parameter, fill = contacts)) +
  geom_tile(color = "white", lwd = 1,linetype = 1)+
  geom_text(aes(label = round(contacts,2)), color = "white", size = 5) +
  ylab('Values Importance') + 
  xlab('Experiential Importance') +
  ggtitle("Average active contacts per HUMAT on the final day\nfor different importance configurations of the HUMAT population") +
...
ggplot(data_for_visualization, aes(x = experiential_importance_parameter, y = values_importance_parameter, fill = dilemmas)) +
  geom_tile(color = "white", lwd = 0.5,linetype = 1)+
  geom_text(aes(label = round(dilemmas,2)), color = "white", size = 5) +
  ylab('Values Importance') + 
  xlab('Experiential Importance') +
  ggtitle("Average number of dilemmas per HUMAT on the final day \nfor different importance configurations of the HUMAT population") +
  scale_x_continuous(breaks = seq(0, 1.0, len = 6)) +
  scale_y_continuous(breaks = seq(0, 1.0, len = 6)) +
...
ggplot(data_for_visualization, aes(x = experiential_importance_parameter, y = values_importance_parameter, fill = opinion_mean)) +
  geom_tile(color = "white", lwd = 0.5,linetype = 1)+
  geom_text(aes(label = round(opinion_mean,2)), color = "white", size = 5) +
  ylab('Values Importance') + 
  xlab('Experiential Importance') +
  ggtitle("Mean opinion on the final day \nfor different importance configurations of the HUMAT population") +
  scale_x_continuous(breaks = seq(0, 1.0, len = 6)) +
  scale_y_continuous(breaks = seq(0, 1.0, len = 6)) +
  scale_fill_continuous(limits=c(0, 1), breaks=seq(0,1,by=0.1))+
  theme_bw() +
...
ggplot(data_for_visualization, aes(x = experiential_importance_parameter, y = values_importance_parameter, fill = sd)) +
  geom_tile(color = "white", lwd = 0.5,linetype = 1)+
  geom_text(aes(label = round(sd,2)), color = "white", size = 5) +
  ylab('Values Importance') + 
  xlab('Experiential Importance') +
  ggtitle("Standard deviation of opinion on the final day\nfor different importance configurations of the HUMAT population") +
  scale_x_continuous(breaks = seq(0, 1.0, len = 6)) +
  scale_y_continuous(breaks = seq(0, 1.0, len = 6)) +
  scale_fill_continuous(limits=c(0, 1), breaks=seq(0,1,by=0.1))+
  theme_bw() +
